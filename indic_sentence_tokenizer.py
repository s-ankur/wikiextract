...
class IndicSentenceTokenizer:
    INDIC='ऀँंःऄअआइईउऊऋऌऍऎएऐऑऒओऔकखगघङचछजझञटठडढणतथदधनऩपफबभमयरऱलळऴवशषसहऺऻ़ऽािीुूृॄॅॆेैॉॊोौ्ॎॏॐ॓॔ॕॖॗक़ख़ग़ज़ड़ढ़फ़य़ॠॡॢॣ०१२३४५६७८९'+string.digits+'\u200d\xa0\u200c'
    ...
    def line_to_sentence(self,line):
        ...
        old_pos=0
        while pos<len(line):
            new_pos = min(filter(lambda x:x!=-1,(line.find(end,pos) for end in self.SENTENCE_END) ),default=len(line)-1)+1
    ...
    def tokenize(self, text, min_tok=6,max_tok = 27,max_non_indic = .2,ret_blank=False):
        ...
        for sentence in self.line_to_sentence(text):
            ...
            if len(sentence) > min_tok and len(sentence)< max_tok and non_indic_ratio < max_non_indic:
